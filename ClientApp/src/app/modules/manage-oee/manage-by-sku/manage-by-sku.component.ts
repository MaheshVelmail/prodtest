...
@Component({
  ...
  templateUrl: './manage-by-sku.component.html',
  styleUrls: ['./manage-by-sku.component.css']
})
export class ManageBySKUComponent implements OnInit {
  columnList = ["productCode", "productDesc", "package", "superPackage", "casePerHourCapacity", "planPercentage", "casePerHour", "action"];
  List: any
  ...
  skuselection = new SelectionModel<ManageBySKUModel>(true, []);
  ...
  getOEERequests(id: any) {
    ...
    this.manageOeeService.getOEERequests(id, this.oEEtype)
      .subscribe(data => {
        ...
        {
          this.disableSelectAllCheckbox=false;
        }
  ...
  deleteOeeRequest() {
    ...
    this.manageOeeService.DeleteOEERequest(this.oeeby, this.deletedData, this.oEEtype, this.removePendingRequestToo)
      .subscribe(response => {
        if (response.data == -1) {
  ...
  updateOEERequest() {
    this.editSelected = "Edit Selected"
    this.spinnerService.show();
    this.showValidationMessage = false;
    ...
    if (IsInvalidPlanPercentage.length != 0) {
      ...
      this.manageOeeService.updateOEERequest(this.editedData, this.oEEtype, this.isYearEndChangeModel)
        .subscribe(() => {
          ...
          error => console.log('Update OEE Request', error)
        );
    }
  }
    ...
    if (this.addOEERequestForm.valid) {
      this.manageOeeService.addOEERequest(this.newSKUData, this.oEEtype, this.isYearEndChangeModel)
        .subscribe(() => {
          ...
          error => console.log('Add OEE Request', error)
        );
  ...
  ResetUserSearchControl(event: any) {
    ...
    this.addOEERequestForm.get('productCode').setValue('');
    this.addOEERequestForm.get('productDesc').setValue('');
    this.addOEERequestForm.get('package').setValue('');
  ...
  selectedUser(selData: any) {
    ...
    this.addOEERequestForm.get('displayName').setValue(selData.productCode + ' || ' + selData.package + ' || ' + selData.superPackage);
    this.addOEERequestForm.get('productId').setValue(selData.productId);
    this.addOEERequestForm.get('productCode').setValue(selData.productCode);
    this.addOEERequestForm.get('productDesc').setValue(selData.productDesc);
    this.addOEERequestForm.get('package').setValue(selData.package);
  ...
  casePerHourmethodForAdd(inputVal: any, data: any, event: any) {
    ...
    if (data == 'casePerHour') {
      ...
      this.addOEERequestForm.get('planPercentage').setValue(parseFloat(((event.target.value / inputVal.casePerHourCapacity) * 100).toFixed(2)));
    }
    else if (data == 'casePerHourCapacity') {
  ...
  keyPressNumbers(event, value: any, input: any) {
    ...
    if (input == 'planPercentage') {
      if (val >= 10 || charCode > 31 && (charCode < 48 || charCode > 57)) {
        this.addOEERequestForm.get('planPercentage').setValue(100);
  ...
  skumasterToggle() {
    this.isSkuAllSelected() ?
      this.skuselection.clear() :
      this.dataSource.data.forEach(element => this.skuselection.select(element));

  }
  isSkuAllSelected() {
    const numSelected = this.skuselection.selected.length;
    const numRows = this.dataSource.data.length;
    return numSelected === numRows;
  }
  selecedskueditAction(e: any) {
    e.editable = true;
    e.lineId = this.lineId;
    e.plantId = this.plantId;
    this.editedData.push(e);
  ...
  editSelectedsku(btnText: string) {
    if (btnText === "Edit Selected") {
      this.editSelected = "Undo Selected"
      this.editSelsku()
    }
    else {
      this.editSelected = "Edit Selected"
      this.skuselection.selected.filter(item => this.undo(item))
      this.skuselection.clear()
    }
  }
  editSelsku() {
    this.skuselection.selected.filter(item => this.selecedskueditAction(item))
  }
  deleteSelectedSkuConfirmation() {
    this.deletedData = [];
    let selectedSku=this.skuselection.selected
    selectedSku.forEach(element => {
      element.lineId=this.selectedline;
      element.plantId=this.selectedplant;
