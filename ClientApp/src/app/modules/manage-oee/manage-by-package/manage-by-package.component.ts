...
@Component({
  selector: 'manage-by-package',
  templateUrl: './manage-by-package.component.html',
  styleUrls: ['./manage-by-package.component.css']
})
export class ManageByPackageComponent implements OnInit {
  columnList = ["package", "superPackage", "casePerHourCapacity", "planPercentage", "casePerHour", "action"];
  List: any
  ...
  getOEERequests(id: any) {
    ...
    this.manageOeeService.getOEERequests(id, this.oEEtype)
      .subscribe(data => {
        let res=data
        this.disableAddbtn = false;
        const Listdata: ManageBySKUModel[] = res;
  ...
  selectedUser(selData: any) {  
    ...
    this.addOEERequestForm.get('displayName').setValue(selData.package + ' || ' + selData.superPackage);
